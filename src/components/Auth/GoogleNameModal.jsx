import React, { useState } from 'react';
import './GoogleNameModal.css';

export function GoogleNameModal({ googleUser, onConfirm, onCancel, error }) {
  const [shareName, setShareName] = useState(true);
  const [customName, setCustomName] = useState(googleUser?.name || '');

  const handleSubmit = (e) => {
    e.preventDefault();
    const finalName = shareName ? (googleUser?.name || 'Google User') : (customName.trim() || 'Spotify Listener');
    onConfirm({
      displayName: finalName,
      shareName,
    });
  };

  return (
    <div className="google-modal-overlay">
      <div className="google-modal-card">
        <div className="google-modal-header">
          <div className="google-icon-wrapper">
            <svg viewBox="0 0 24 24" width="32" height="32">
              <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
              <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
              <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l2.85-2.22.81-.63z"/>
              <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84c.87-2.6 3.3-4.52 6.16-4.52z"/>
            </svg>
          </div>
          <h3>Share your name with Spotify Clone?</h3>
          <p className="google-modal-sub">
            Logged in as <strong>{googleUser?.email}</strong>
          </p>
        </div>

        <form onSubmit={handleSubmit} className="google-modal-body">
          <div 
            className={`option-card ${shareName ? 'selected' : ''}`}
            onClick={() => setShareName(true)}
          >
            <input
              type="radio"
              id="share-yes"
              name="shareName"
              checked={shareName}
              onChange={() => setShareName(true)}
            />
            <label htmlFor="share-yes">
              <span className="option-title">Share my Google name</span>
              <span className="option-desc">Display as "{googleUser?.name || 'Google User'}" on your profile & dashboard</span>
            </label>
          </div>

          <div 
            className={`option-card ${!shareName ? 'selected' : ''}`}
            onClick={() => setShareName(false)}
          >
            <input
              type="radio"
              id="share-no"
              name="shareName"
              checked={!shareName}
              onChange={() => setShareName(false)}
            />
            <label htmlFor="share-no">
              <span className="option-title">Keep name private / Choose display name</span>
              <span className="option-desc">Hide your full name and use a custom nickname</span>
            </label>
          </div>

          {!shareName && (
            <div className="custom-name-input-group">
              <label htmlFor="customNameInput">Display Nickname</label>
              <input
                id="customNameInput"
                type="text"
                value={customName}
                onChange={(e) => setCustomName(e.target.value)}
                placeholder="e.g. Spotify Listener"
                maxLength={40}
              />
            </div>
          )}

          {error && (
            <div className="modal-error-message">
              {error}
            </div>
          )}

          <div className="google-modal-actions">
            {onCancel && (
              <button type="button" className="btn-cancel" onClick={onCancel}>
                Cancel
              </button>
            )}
            <button type="submit" className="btn-confirm">
              Continue
            </button>
          </div>
        </form>
      </div>
    </div>
  );
}
